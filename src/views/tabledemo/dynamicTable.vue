<template>
  <div class="tablebox">
    ...
    <el-table :data="tableData" style="width: 100%" :header-cell-style="rowCellClass" header-align="center">
      ...
      <el-table-column width="80" align:center>
        <template slot-scope="scope">
          ...
          <i v-show="tableData.length !== 1" class="iconfont icon-decrease" @click="decrease(scope.$index)"></i>
          <i v-show="scope.$index === tableData.length - 1" class="iconfont icon-add-2" @click="add(scope.$index)"></i>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    initTable () {
      this.tableData.push(this.tableRowData)
    },
    decrease (index) {
      console.log('减号')
      console.log(index)
    ...
    add (index) {
      console.log('加号')
      console.log(index)
      this.tableData.push({ medicaName: '', dose: '', unit: '', count: '', frequency: '', days: '' })
      // 在对应的index 后 插入一条数据
    },
